...
int options(int ac, char ** av, Options& opts)
{
  ...
  desc.add_options()("intrinsics,K", po::value<string>(&opts.k_file),
                     "The camera intrinsics file, should be yaml and have atleast \"K:...\". Required.");
  desc.add_options()("distortion,D", po::value<string>(&opts.d_file)->default_value(""),
                     "The camera distortion file, should be yaml and have atleast \"D:...\". optional, no distortion assumed if not given.");
  desc.add_options()("directory,d", po::value<string>(&opts.directory)->default_value("./"),
                     "The directory that the images to stitch are in");
  desc.add_options()("stitch_output,o", po::value<string>(&opts.stitch_output)->default_value("stitched.jpg"),
                     "the full path to the final stitch image, should have image extension. (.jpg,.png)");
  desc.add_options()("images", po::value<vector<string> >(&opts.image_names), "input images.");
...
bool stitch(const std::string& pano_directory_name, const std::string& stitch_name, StitchProgressCallable* callback =
    NULL)
{
  try
  {
